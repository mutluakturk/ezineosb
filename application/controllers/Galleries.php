...
class Galleries extends CI_Controller
{
                    ...
                    if ($gallery_type != "video") {
                        if (!mkdir($path, 0755)) {
                            $alert = array(
                                ...
                                "text" => "Galeri Oluşturulamadı! (Yetkisiz Giriş Denemesi)",
                                "type" => "error"
                            );
    ...
    public function update($id, $gallery_type, $oldFolderName = "")
    {
        ...
        if ($validate) {
            ...
            if ($_FILES["img_url"]["name"] == "") {
                ...
                //resim gönderildi ise yapılacaklar
                ...
                if ($upload) {
                    $uploaded_file = $this->upload->data("file_name");
                    //eger resim yüklendi ise
                    //önceki resmi silelim
                    $gal = $this->gallery_model->get(array("id" => $id));
                    $imgPath = "uploads/galleries_v/" . $gal->img_url;
                    unlink($imgPath);
    ...
    public function isActiveSetter($id)
    {
        if ($id) {
            ...
            $this->gallery_model->update(array("id" => $id), array("isActive" => $isActive));
        }
    }
    ...
    public function fileIsActiveSetter($id, $gallery_type)
    {
        if ($id && $gallery_type) {
            $modelName = ($gallery_type == "image") ? "image_model" : "file_model";
            $isActive = ($this->input->post("data") === "true") ? 1 : 0;
            $this->$modelName->update(array("id" => $id), array("isActive" => $isActive));
        }
    }
    ...
    public function upload_form($id)
    {
        ...
        if ($item->gallery_type == "image") {
            $viewData->items = $this->image_model->get_all(array("gallery_id" => $id), "rank ASC");
        } else if ($item->gallery_type == "file") {

            $viewData->items = $this->file_model->get_all(array("gallery_id" => $id), "rank ASC");
        } else {
            $viewData->items = $this->video_model->get_all(array("gallery_id" => $id), "rank ASC");
        }
    ...
    public function file_upload($gallery_id, $gallery_type, $folderName)
    {
        ...
        $config["allowed_types"] = "jpg|jpeg|png|JPG|JPEG|PNG|pdf|PDF|doc|DOC|docx|DOCX|xls|XLSX";
    ...
    public function refresh_file_list($gallery_id, $gallery_type)
    {
        ...
        $viewData->items = $this->$modelName->get_all(array("gallery_id" => $gallery_id));
    ...
    public function gallery_video_list($id)
    {
        ...
        /** tablodan verilerin getirilmesi */
        $items = $this->video_model->get_all(array("gallery_id" => $id), "rank ASC");
    ...
    public function update_gallery_video_form($id)
    {
        ...
        $item = $this->video_model->get(array("id" => $id));
    ...
    public function galleryVideoIsActiveSetter($id)
    {
        if ($id) {
            $isActive = ($this->input->post("data") === "true") ? 1 : 0;
            $this->video_model->update(array("id" => $id), array("isActive" => $isActive));
        }
    }
    ...
    public function galleryVideoDelete($id, $gallery_id)
    {
        ...
        $delete = $this->video_model->delete(array("id" => $id));
